...
import {
  LayoutDashboard, Brain, ClipboardList, Trophy, Coins, ArrowRight,
} from "lucide-react";
...
export default function PlatformPreview() {
  ...
  const features = [
    { title: p.dashboardTitle, description: p.dashboardDesc, color: "blue" as const },
    { title: p.mistakeTitle, description: p.mistakeDesc, color: "purple" as const },
    { title: p.mocksTitle, description: p.mocksDesc, color: "emerald" as const },
    { title: p.leaderTitle, description: p.leaderDesc, color: "amber" as const },
    { title: p.pointsTitle, description: p.pointsDesc, color: "orange" as const },
  ];

  return (
    <section className="py-24 overflow-hidden bg-white dark:bg-slate-900">
        ...
        >
          <div className="bg-navy-950 dark:bg-slate-950 rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
            ...
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-white/10">
              ...
              <div className="flex-1 text-center text-xs text-white/30">{p.browserBar}</div>
            </div>
            ...
            <div className="p-5 grid grid-cols-12 gap-4 min-h-[360px]">
              ...
              <div className="col-span-2 hidden md:flex flex-col gap-1">
                {p.appMenu.map((label, i) => (
                  ...
                  >
                    {React.createElement(featureIcons[i] || LayoutDashboard, { size: 13 })}
              ...
              <div className="col-span-12 md:col-span-7 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={s.label} className="bg-white/5 rounded-xl p-3 border border-white/10">
                      ...
                      <div className={`text-xs font-medium ${s.up ? "text-emerald-400" : "text-white/40"}`}>
                        {s.sub}
                      </div>
                    </div>
                  ))}
                ...
                <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                  <div className="text-white/50 text-xs mb-2">{p.scoreHistory}</div>
                ...
                <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                  <div className="text-white/50 text-xs mb-2.5">{p.recentMistakes}</div>
                  <div className="space-y-1.5">
                    {["Algebra — Quadratics", "Reading — Inference", "Writing — Transitions"].map((m) => (
                      <div key={m} className="flex items-center gap-2 text-xs text-white/50">
              ...
              <div className="col-span-12 md:col-span-3 space-y-3">
                  ...
                  {[
                    ...
                    <div
                      ...
                      className={`flex items-center justify-between text-xs px-2 py-1.5 rounded-lg mb-1 ${
                        l.you ? "bg-blue-600/20 border border-blue-500/30" : ""
                      }`}
                    >
                      <div className="flex items-center gap-1.5">
                        <span className="text-white/30 w-4 text-center">{l.rank}</span>
                ...
                <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                  ...
                  <div className="text-white/30 text-xs mt-1">550 {p.toNextPrize}</div>
                </div>
              </div>
            </div>
          </div>
